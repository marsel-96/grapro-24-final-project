...
class UniformBufferObject final: public IndexedBufferObjectBase<IndexedBufferObject::UniformBuffer>
{
public:
    UniformBufferObject();
...
template<typename T>
void UniformBufferObject::AllocateData(const size_t elementCount, const Usage usage)
{
...
template<typename T>
void UniformBufferObject::AllocateData(std::span<const T> data, Usage usage)
{
...
// Call the base implementation with the span converted to bytes
template<typename T>
void UniformBufferObject::UpdateData(std::span<const T> data, const size_t offsetBytes)
{
